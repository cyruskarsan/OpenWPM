...
class Browser:
    ...
    def launch_browser_manager(self):
        ...
        """
        self.logger.info("BROWSER %i: Launching browser..." % self.crawl_id)
        tempdir = None
    ...
    def close_browser_manager(self, force: bool = False):
        ...
        """
        self.logger.debug("BROWSER %i: Closing browser..." % self.crawl_id)
        ...
        if not in_command_thread and self.command_thread is not None:
            ...
            if self.command_thread.is_alive():
                self.logger.debug(
                    ...
                    "Assuming the browser process is locked..." % self.crawl_id
                )
                self.kill_browser_manager()
                return
        ...
        if self.command_queue is None:
            self.logger.debug(
                "BROWSER %i: Command queue not found while closing." % self.crawl_id
            )
            self.kill_browser_manager()
            return
        ...
        if status != "OK":
            self.logger.debug(
                "BROWSER %i: Command failure while closing browser." % self.crawl_id
            )
            self.kill_browser_manager()
            return
        ...
        if self.browser_manager.is_alive():
            self.logger.debug(
                ...
                "after executing shutdown command." % self.crawl_id
            )
            self.kill_browser_manager()
            return

        self.logger.debug(
            "BROWSER %i: Browser manager closed successfully." % self.crawl_id
        )
    ...
    def kill_browser_manager(self):
        ...
        self.logger.debug(
            ...
            "Browser PID: %s"
            % (self.crawl_id, self.browser_manager.pid, self.browser_pid)
        )
    ...
    def shutdown_browser(self, during_init: bool, force: bool = False) -> None:
        ...
        self.logger.debug("BROWSER %i: Closing browser manager..." % self.crawl_id)
        ...
        if not during_init and self.browser_params["profile_archive_dir"] is not None:
            self.logger.warning(
                ...
                "See: https://github.com/mozilla/OpenWPM/projects/2" % self.crawl_id
            )
        """
        self.logger.debug(
